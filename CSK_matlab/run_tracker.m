...
for frame = 1:numel(img_files),
    ...
    %重构POS，因为行列和坐标是相反的,而且需要得到target_sz，前面用的一直是sz
    
	if frame == 1,  %first frame, create GUI
		figure( 'Name',['Tracker- ' video_path])      %这里改了有点，源代码给出的number之类的没有给参数，就省去了
		im_handle = imshow(im, 'Border','tight', 'InitialMag',200);
		rect_handle = rectangle('Position',rect_position, 'EdgeColor','g');   %红色跟踪框
	else
		try    %subsequent frames, update GUI  后续帧更新GUI
