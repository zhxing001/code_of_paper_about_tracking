...
void grad1( float *I, float *Gx, float *Gy, int h, int w, int x ) 
{
  ...
  Ip=I-h; In=I+h; r=.5f;
  if(x==0)
  { r=1; Ip+=h; } else if(x==w-1) { r=1; In-=h; }
...
void gradMag( float *I, float *M, float *O, int h, int w, int d, bool full ) {
  int x, y, y1, c, h4, s; 
  float *Gx, *Gy, *M2; __m128 *_Gx, *_Gy, *_M2, _m;
...
// normalize gradient magnitude at each location (uses sse)   //归一化梯度大小
  ...
  int bin, int nOrients, int softBin, bool full )
{
  ...
  // normalize boundary bins which only get 7/8 of weight of interior bins，归一化边界
