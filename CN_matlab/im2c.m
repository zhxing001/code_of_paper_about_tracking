...
if(nargin<3)       %输入参数小于3的话就把color置位0
   color=0;
end

RR=im(:,:,1);GG=im(:,:,2);BB=im(:,:,3);                                    %复制一份备用


index_im = 1+floor(RR(:)/8)+32*floor(GG(:)/8)+32*32*floor(BB(:)/8);        %每个通道都拉成一列，然后1+RR/8+32*RR/8+32*32*BB/8   不过不知道为什么这么做,这样算出来的是一个矩阵（width*heigth*1）
